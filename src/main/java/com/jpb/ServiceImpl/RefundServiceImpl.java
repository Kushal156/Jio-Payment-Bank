...
public class RefundServiceImpl implements RefundService {
	...
	public ResponseEntity<?> voucherVerify(CustomerInputRequestDTO input, HttpServletRequest httpRequest) {
		...
		try {
			...
			if ("SUCCESS".equalsIgnoreCase(finalResponse.getStatus())) {
				...
				String voucherORN = responseVoucher != null ? responseVoucher.getVoucherOrn() : null;
				String amount = responseVoucher != null ? responseVoucher.getNetAmount() : null;
				
				log.info("Voucher ORN :: {}, Amount :: {}", voucherORN, amount);

				refundMaster.setAmount(amount);

				Optional<CustomerMasterEntity> masterEntity = masterRepo.findByApplicationNumber(voucherORN);

				if (masterEntity.isEmpty()) {
					error.setCode("200");
					error.setMessage("Customer not found with same Application Number");
					finalResponse.setError(error);
					return ResponseEntity.ok(finalResponse);
				}
				...
				if (masterEntity.isPresent()) {
					...
					refundMaster.setMasterExternalAppRefNumber(master.getApplicationNumber());
					refundMaster.setMasterId(masterId);
					refundMaster.setVakTransactionId(master.getTransactionId()!= null ? master.getTransactionId() : null);
				}
	...
	public ResponseEntity<?> voucherRedeem(CustomerInputRequestDTO input, HttpServletRequest httpRequest) {
		...
		try {
			...
			if (masterEntity.isPresent()) {
				...
				vakTransactionId = refundMaster.getVakTransactionId();
				amount = Double.parseDouble(refundMaster.getAmount());
				
				log.info("Vakrangee Transaction ID :: {}, Refund Amount :: {}", vakTransactionId, amount);
				...
				if ("SUCCESS".equalsIgnoreCase(finalResponse.getStatus())) {
					...
					refundMaster.setVakCreditStatus(refundStatus);
	...
	// Voucher Resend
	@Override
	public ResponseEntity<?> voucherResend(CustomerInputRequestDTO input, HttpServletRequest httpRequest) {
		...
		try {
			...
			try {
				response = rest.exchange(URL, HttpMethod.POST, entity, String.class);
				log.info("Resend Refund Voucher Code Raw Response :: {}", response.getBody());
				responseBody = response.getBody();
				statusCode = response.getStatusCode().value();
	...
	@Override
	public ResponseEntity<?> customerVoucherFailed(CustomerInputRequestDTO input, HttpServletRequest httpRequest) {
		...
		try {
			log.info("Customer Voucher Search Failed Request from Customer :: {}", input.toString());
			...
			request.setMobile(input.getMobileNumber());
			request.setPan(input.getPanNo());
			request.setPage(0);
			request.setSize(10);
			
			log.info("JSON Request for Customer ORN Search Failed :: {}", mapper.writeValueAsString(request));
			...
			log.error("Customer Refund Voucher Search Failed Exception", e);
